...
edf <- data.frame(elecdf[which(elecdf$Date=='2/1/2007' | elecdf$Date=='2/2/2007'),])
...
datetimeformats <- strptime(edf$DateTime,format="%m/%d/%Y %H:%M:%S")
...
hist(edf$Global_active_power_numeric, col="red", 
     xlab="Global Active Power (kilowatts)", 
     ylab="Frequency", main="Global Active Power", axes=FALSE)
